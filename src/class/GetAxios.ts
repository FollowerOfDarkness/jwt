...
export default class GetAxios {
  ...
  async login(email: string, pass: string) {
    const serverResonse = await this.axiosAuthServer.post<
  ...
  private decodeToken(token: unknown): AllPayload | null {
    if (typeof token === "string") {
      ...
      try {
        const encodeToken =
          payloadBase64 && Buffer.from(payloadBase64, "base64").toString()
  ...
  private async refrash() {
    this.clearAccessToken()
    const serverResonse = await this.axiosAuthServer.post("/refresh")
  ...
  private async checkExpTryRefrash() {
    if ((this.payload?.exp ?? 0 + 5) < Math.round(new Date().getTime() / 1000)) {
      await this.refrash()
    }
  }
  ...
  async getPayload(): Promise<ActualPayload> {
    ...
    const { exp, iat, ...payload } = <AllPayload>this.payload
    return payload
  }
